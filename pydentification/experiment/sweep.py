...
def run_sweep_step(
    ...
    with wandb.init(reinit=True):
        ...
        try:
            ...
            model, trainer = context.model_fn(project_name, experiment_config["training"], parameters)
...
def main(data: str, experiment: str, context: RuntimeContext):
    ...
    run_sweep_fn = partial(
        run_sweep_step, context, experiment_config["general"]["project"], dataset_config, experiment_config
    )
    wandb.agent(
        sweep_id,
